...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        for i in range(len(input)):
            grad = -np.outer(self.output[i], self.output[i])
            grad += np.diag(self.output[i])
            self.gradInput[i] = gradOutput[i] @ grad
        return self.gradInput
...
class LogSoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        self.gradInput = np.zeros(gradOutput.shape)
        for i in range(len(input)):
            grad = np.eye(self.output.shape[1]) - np.exp(self.output)[i]
            self.gradInput[i] = gradOutput[i] @ grad
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        batch_variance = np.var(input, axis=0)
        grad = len(input) * gradOutput - np.sum(gradOutput, axis=0, keepdims=True)
        grad -= self.output * np.sum(gradOutput * self.output, axis=0, keepdims=True)
        self.gradInput = grad / (len(input) * np.sqrt(batch_variance + self.EPS))
        return self.gradInput
...
class ELU(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        self.gradInput = np.where(input > 0, 1, self.alpha + self.output) * gradOutput
        return self.gradInput
...
class ClassNLLCriterionUnstable(Criterion):
    ...
    def updateGradInput(self, input, target):
        ...
        self.gradInput = -1 / (input_clamp * target.shape[0]) * target
        return self.gradInput
